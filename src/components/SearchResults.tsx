...
export default function SearchResults({ query, results }) {
  ...
  return (
    <div className="absolute top-14 right-4 rounded-md w-1/3 bg-slate-100 text-gray-500 shadow-lg p-1">
      {results.length > 0 ? (
        <ul>
          {results.map((result) => (
            <li key={result.id} className="flex flex-col">
              <Link
                className="flex flex-col bg-white text-gray-500 p-2 m-1 shadow rounded hover:bg-blue-500 hover:text-white"
                to={result.slug}
              >
